...
<body onload='document.all.username.focus();'>
	<div class="wrapper">
		<div class="google-header-bar">
			...
			<a href="" title="" style="text-decoration: none;">
				...
				<span class="signup-button">Δεν είστε χρήστης; 
					...
					href="/stelexi/signup?form&returnView=admin">Δημιουργηστε λογαριασμο</a>
				</span>
			</div>
		</div>
		...
		<div class="errorblock">
			Η προσπάθεια σύνδεσής σας δεν ήταν επιτυχής. Προσπαθήστε ξανά.<br />
		</div>
		</c:if>
			...
			${message}
				<div class="signin-box">
					...
					<form id="signupForm"
						action="/stelexi/signup"
						method="post">
						<label> <strong
							class="email-label">Όνομα χρήστη</strong>
							<c:if test="${!editMode}">
						...
						<label> <strong class="passwd-label">Κωδικός</strong> 
							<input type="password" name="password" id="Passwd" value="${user.password}">
						</label>
						<label> <strong class="passwd-label">Επιβεβαίωση Κωδικού</strong> 
							<input type="password" name="passwordRewrite" id="passwordRewrite" value="">
						</label>
						
						<label> <strong class="passwd-label">Επίπεδο ασφαλείας χρήστη</strong> 
							<select name="priviledges" class="">
								<option value="2" ${2 == user.priviledges ? 'selected' : ''}>Μόνο Ανάγνωση δεδομένων</option>
								<option value="1" ${1 == user.priviledges ? 'selected' : ''}>Ανάγνωση & εγγραφή/επεξεργασία δεδομένων</option>
							</select>
						</label>
						<a href="/stelexi/login?form"><input type="submit" class="g-button g-button-submit"
							name="backToLogin" id="backToLogin" value="Πίσω"></a> 
						 
						<input id="editModeParam" name="editMode" type="hidden" value="${editMode}" />

						<input id="returnViewParam" name="returnView" type="hidden" value="${returnView}" />
						<input id="createModeParam" name="createMode" type="hidden" value="${createMode}" />
						
						<c:if test="${editMode}">
							<input id="usernameParam" name="username" type="hidden" value="${user.username}" />
						</c:if>
						
						<input id="userIdParam" name="userId" type="hidden" value="${user.id}" />
						 
						<c:choose>
						<c:when test="${editMode}">
							<input type="submit" class="g-button g-button-submit" name="signIn" id="signIn" value="Αποθήκευση">
						</c:when>
						<c:otherwise>
							<input type="submit" class="g-button g-button-submit" name="signIn" id="signIn" value="Δημιουργία χρήστη">
						</c:otherwise>
						</c:choose> 
					</form>
					<ul>
...
<script type="text/javascript">
	...
	el.innerHTML = "© " + d.getFullYear() + " Γνώσις";
</script>
