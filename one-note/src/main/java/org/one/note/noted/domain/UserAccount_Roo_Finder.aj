...
privileged aspect UserAccount_Roo_Finder {
    ...
    public static TypedQuery<UserAccount> UserAccount.findUserAccountsByEmailEqualsAndPasswordEquals(String email, String password) {
        if (email == null || email.length() == 0) throw new IllegalArgumentException("The email argument is required");
        if (password == null || password.length() == 0) throw new IllegalArgumentException("The password argument is required");
        EntityManager em = UserAccount.entityManager();
        TypedQuery<UserAccount> q = em.createQuery("SELECT o FROM UserAccount AS o WHERE o.email = :email  AND o.password = :password", UserAccount.class);
        q.setParameter("email", email);
